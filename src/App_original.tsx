...
const CarJourneyLanding = React.lazy(async () => {
  const mod = await import('./components/scene/CarJourneyLanding');
...
const AppContent: React.FC = () => {
  ...
  return (
    <>
      <nav className={`fixed w-full z-50 top-0 transition-all duration-500 ${scrolled ? 'py-2' : 'py-4'}`}>
        <motion.div 
          className={`mx-3 sm:mx-5 lg:mx-8 transition-all duration-500 rounded-2xl border ${
            scrolled 
              ? 'py-3 bg-slate-950/72 border-white/12 shadow-[0_22px_60px_rgba(2,6,23,0.45)] backdrop-blur-2xl' 
              : 'py-4 bg-slate-950/40 border-white/8 backdrop-blur-xl'
          }`}
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex justify-between items-center">
              <Link to="/" className="group inline-flex items-center gap-3" onClick={closeMobileMenu}>
                <span className="w-9 h-9 rounded-xl border border-cyan-200/30 bg-cyan-400/15 text-cyan-100 text-xs font-bold flex items-center justify-center shadow-[0_0_20px_rgba(56,189,248,0.25)]">
                  MS
                </span>
                <span className="text-white font-semibold tracking-wide group-hover:text-cyan-100 transition-colors">
                  Meet Shah
                </span>
              </Link>

              <div className="hidden md:flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-2 py-1.5">
                {navItems.map((item) => (
                  <NavLink
                    key={item.to}
                    to={item.to}
                    className={({ isActive }) =>
                      `px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                        isActive
                          ? 'bg-cyan-400/15 border border-cyan-300/30 text-cyan-100'
              ...
              <button
                className="md:hidden text-white/65 hover:text-white transition-colors w-10 h-10 rounded-xl border border-white/15 bg-white/[0.03] flex items-center justify-center"
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (
              ...
              >
                <div className="px-4 pb-5 pt-3 grid grid-cols-2 gap-2 border-t border-white/10">
                  {navItems.map((item) => (
                    <NavLink
                      key={item.to}
                      to={item.to}
                      ...
                      className={({ isActive }) =>
                        `px-4 py-3 rounded-xl text-sm font-medium text-center transition-colors ${
                          isActive
                            ? 'bg-cyan-400/15 border border-cyan-300/30 text-cyan-100'
